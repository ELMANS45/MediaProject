...
export default function MyProfile() {
    const {userForm , getProfile,getAllPosts, profilePosts} = useContext(ProfileContext)
    ...
    useEffect(()=>{
        const fetch = async ()=>{
			try {
				await getProfile()
				if(userForm?._id)[
					await getAllPosts(userForm._id)
				]
			}
			catch(error){
				console.log(error);
		...
		fetch(); 
    },[userForm ,userForm?.photo])
  return (
    <>	
		{isLoading ? <Loading/> : <>            <div className="container flex justify-center items-center my-auto mt-20" key={userForm?._id}>
            <div className="flex flex-col justify-center max-w-xs p-6 shadow-md rounded-xl sm:px-12 dark:bg-gray-50 dark:text-gray-800">
	<img src={userForm?.photo} alt="" className="w-32 h-32 mx-auto rounded-full dark:bg-gray-500 aspect-square" />
	<div className="space-y-4 text-center divide-y dark:divide-gray-300">
		<div className="my-2 space-y-1">
			<h2 className="text-xl font-semibold sm:text-2xl">{userForm?.name}</h2>
		</div>
		<div className="flex justify-center pt-2 space-x-4 align-center">
            ...
            <div className="container flex justify-center flex-col gap-5 items-center">
				<Link to="/addpost"><button className= ' mt-5 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus: ring-blue-800'>Add Post</button></Link>
                <Posts profilePosts = {profilePosts}/>
            </div> </>}
    </>
  )
}
